...
**4. DEK/KEK (Key Encryption Key) envelope encryption with rotation proportional to document count.** Each document holds a data encryption key (DEK). Each role holds a key encryption key (KEK) that wraps the DEKs for documents that role can access. When a role is revoked, the KEK rotates, and DEKs for affected documents are re-wrapped under the new KEK. An attacker who compromises a role KEK gains access to documents that key could decrypt. Not documents outside that role's scope. Not forward access after rotation. Okonkwo's key hierarchy requirement — root org key to role KEKs to per-node wrapped keys to per-record DEKs — codifies exactly this structure.
