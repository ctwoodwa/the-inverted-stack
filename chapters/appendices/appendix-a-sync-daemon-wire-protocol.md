...
**Data-protection note.** `role_claims` and `node_id` may constitute personal data under GDPR Article 4(1), UK GDPR, LGPD Article 5, India's DPDP Act 2023, and parallel regimes (UAE DPL, NDPR, POPIA, PIPL, Japan PIPA, Korea PIPA — see Appendix F) where the node identity is attributable to an identifiable natural person. Implementers MUST evaluate the bundle contents under the applicable regime. The QR transfer channel itself is assumed untrusted. The relay's signature verification provides integrity. Confidentiality of the bundle during transfer is the implementer's responsibility — in practice, by scanning the QR code inside a trusted physical perimeter rather than from photographs or remote displays.
