...
The protections in Ch15 §In-Memory Key Handling assume the OS is honest. This section examines what happens when it is not. P6 makes a strong claim: data is encrypted at rest and in transit, keys never leave the node unencrypted, and the relay sees only ciphertext. That claim holds against a network adversary. It does not hold unchanged against an endpoint adversary.

The distinction is load-bearing. A user who reads only the P6 summary and concludes that their data is safe even when their phone is running Pegasus has drawn an incorrect inference. The architecture is responsible for making the correct inference available. This section adds the endpoint-compromise row to the master taxonomy in Ch15 §Threat Model and fills it in.
...
The attestation surface integrates at the sync daemon's handshake layer. Ch14 §Sync Daemon Protocol specifies role-attestation validation at the CAPABILITY_NEG phase, where each subscription request carries a role attestation token signed by the node's device key and is verified before any subscription is granted. Boot attestation reuses the same handshake hook with a different attestation type — a TPM-issued integrity quote rather than a role assertion. The boot-attestation wire format is a forward dependency that Ch14 will document in a future amendment; the integration point and rejection-code surface (`MISSING_ATTESTATION`, `EXPIRED_ATTESTATION`, `INVALID_SIGNATURE`) are already in place. `Sunfish.Kernel.Security` exposes the attestation; relay-side enforcement is in the relay's handshake policy, not the node package.
...
<!-- Phase 7 placeholder: Ch23's share of the Ch15 reference list (~600 words). Citations split per-section between Ch15 (architecture share), Ch22 (key-lifecycle ops share), and Ch23 (endpoint+collaborator+custody ops share). Per UPF Phase 1 triage + Phase 5 R-O23 routing: SGX/TEE family [22]–[24] (endpoint compromise), Pegasus + Hermit [26]–[27] (endpoint compromise), TSP RFC 3161 [28] (chain-of-custody timestamp), Bayou [reference TBD] (subscription model precedent for §Offline Revocation), and any cites referenced from §Collaborator Revocation, §Chain-of-Custody, and §Event-Triggered Re-classification. Final assignment confirmed during Phase 7 reference-list split (post Ch23 split). -->
