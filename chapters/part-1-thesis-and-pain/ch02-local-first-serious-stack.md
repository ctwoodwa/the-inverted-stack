...
Where Obsidian stops is structured data and collaboration. Markdown files have no conflict resolution strategy: when two devices modify the same file concurrently, Obsidian's sync service produces a duplicate — an "Obsidian Sync conflict" copy sitting alongside the original, with no automated merge. Resolution is manual. For a solo note-taker, this is an infrequent and tolerable annoyance. For a team using shared notes to track client work, project status, or decisions — where concurrent edits are the norm — the duplicate-file model fails. There is no CRDT underneath Obsidian's sync. The conflict strategy is to tell the user there is a conflict and let them figure it out.

<!-- CLAIM: Obsidian Sync produces a duplicate file on concurrent edits rather than merging — verify against Obsidian documentation -->
...
<!-- CLAIM: Linear has no peer-to-peer sync mode; the relay is Linear's infrastructure — verify against Linear engineering documentation -->
...
<!-- CLAIM: Actual Budget uses SQLite as its local storage format, making the data accessible to other tools — verify against Actual Budget documentation -->
...
The gap between Automerge and a deployable production system is significant and intentional. Automerge is a library that operates on documents. It assumes the existence of a sync transport — something to move operations between peers. Several sync backends exist (the Automerge sync server, AutomergeRepo), and they work correctly. What they do not provide is a production deployment model for end-user software: enterprise governance, per-role access control, CP-class record types that require distributed lease coordination, financial correctness guarantees, key management at scale, MDM-compatible installers, or a business model.
