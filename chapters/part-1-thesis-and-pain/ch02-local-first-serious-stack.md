...
### The Lightweight Replica Apps (Linear ([linear.app](https://linear.app/), the issue tracker), Liveblocks)
...
Each approach takes local-first seriously in one layer and builds on a centralized dependency in another. Obsidian chose plain files for durability and sacrificed structured collaboration. Linear built a local replica for latency and left authority on the server. Replicache built a sync framework and left the rest to the developer's server. Actual Budget delivered full local authority for a single user and stopped short of team sync. Automerge built correct CRDT merge and left the production deployment model to application builders. Each dependency reflects a real problem the approach did not attempt to solve.

The pattern becomes clearest in a like-for-like comparison across the four axes that determine whether a system meets a serious local-first bar:

| System | Deployment topology | Data-ownership stance | Threat model | Business model |
|---|---|---|---|---|
| **Obsidian** | Local files + optional sync service | Plain markdown on user's filesystem | User-device only; sync service stores opaque conflict copies | Subscription for sync; free for local |
| **Notion** | Server-authoritative web app + offline cache | Vendor-held; user has read-cache | Vendor-side covers all data | Per-seat SaaS subscription |
| **Linear** | Server-authoritative + local SQLite replica | Vendor-held; local replica is a cache | Vendor-side breach exposes all team data | Per-seat SaaS subscription |
| **Replicache** | Server-authoritative + reactive client cache | Vendor-held; framework leaves choice to developer | Vendor-side | Framework license + developer's choice |
| **ElectricSQL** | Server-authoritative Postgres + selective sync | Vendor-held; local SQLite is a filtered replica | Vendor-side for source of truth | Open-source core + commercial Electric service |
| **PowerSync** | Server-authoritative + per-user sync rules | Vendor-held; client holds derived view | Vendor-side | Open-source core + hosted service |
| **Actual Budget** | Fully local + optional self-hosted sync | User-held SQLite | User-device only | Open-source; user runs everything |
| **Automerge** | Library + sync transport (developer-supplied) | Whatever the application chooses | Whatever the application chooses | Open-source library |

The table makes the gap visible. Every system that satisfies vendor-independent data ownership stops short of team collaboration; every system that supports team collaboration delegates authority to a vendor. The missing step is not a better sync library, a more sophisticated CRDT, or a more polished local database. It is the composition of all the layers into a complete node — the composition that no system in this table currently delivers.
...
The regulatory pressure is now global, and the laws cluster by region. European regulation centers on the 2020 Schrems II ruling [4], which constrained transfers of EU personal data to US cloud providers without supplemental safeguards — making local-first residency a structural mechanism that addresses the data-transfer leg of GDPR analysis rather than an architectural preference, with national implementation guidance from Germany's BSI (Bundesamt für Sicherheit in der Informationstechnik) and France's CNIL (Commission nationale de l'informatique et des libertés). India's DPDP (Digital Personal Data Protection) Act 2023 [5], the UAE's DIFC (Dubai International Financial Centre) DPL (Data Protection Law) 2020 [6] — which constrains processing and cross-border transfers of personal data by DIFC-licensed entities and requires specific lawful bases for transfers to non-adequate jurisdictions — and Russia's Federal Law 242-FZ [7] — among the early general-purpose data localization laws globally — are representative of the parallel pattern across GCC (Gulf Cooperation Council), APAC (Asia-Pacific), African, and Americas markets; the full coverage matrix is in Appendix F. In the United States, HIPAA (Health Insurance Portability and Accountability Act) and SOC 2 frame the same structural argument through the healthcare and vendor-audit lenses. In each jurisdiction, an architecture where data lives on the user's own hardware is the architecture that makes compliance tractable.
...
- An enterprise deployment model: MDM-compatible installers, SBOM (Software Bill of Materials) generation, code signing and notarization, air-gap operation, incident response runbooks.
...
[4] Court of Justice of the European Union, "Judgment in Case C-311/18, Data Protection Commissioner v. Facebook Ireland Ltd and Maximillian Schrems," Jul. 16, 2020. [Online]. Available: https://curia.europa.eu/

[5] Government of India, Ministry of Electronics and Information Technology, "The Digital Personal Data Protection Act, 2023," Aug. 11, 2023. [Online]. Available: https://www.meity.gov.in/

[6] Dubai International Financial Centre Authority, "DIFC Data Protection Law, DIFC Law No. 5 of 2020," 2020. [Online]. Available: https://www.difc.ae/

[7] Russian Federation, "Federal Law No. 242-FZ on Amendments to Certain Legislative Acts of the Russian Federation Regarding Clarification of the Procedure for Processing Personal Data in Information and Telecommunications Networks," Jul. 21, 2014, in force Sep. 1, 2015.

[8] Linear, "Scaling the Linear Sync Engine," *linear.app/blog*, 2023. [Online]. Available: https://linear.app/blog/scaling-the-linear-sync-engine

[9] Rocicorp, "Replicache documentation," 2024. [Online]. Available: https://replicache.dev/

[10] "Adobe halts new sales in Russia after invasion of Ukraine," *Reuters*, Mar. 4, 2022. [Online]. Available: https://www.reuters.com/

[11] "Figma blocks Russia-based users in compliance with U.S. sanctions," *TechCrunch*, Mar. 2022. [Online]. Available: https://techcrunch.com/
