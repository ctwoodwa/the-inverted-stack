...
**Automerge ([github.com/automerge/automerge](https://github.com/automerge/automerge), a JSON-like CRDT library)** is excluded not because its design is inferior — the Rust core is a strong reference implementation [5] — but because it lacks a first-class .NET binding covering the sync protocol surface. The decision is a binding gap, not a model gap. It is reversible the moment a covering binding ships.
