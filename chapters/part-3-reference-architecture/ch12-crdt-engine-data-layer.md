...
The tier assignment lives in the plugin’s `IStreamDefinition`. A plugin author who assigns a financial posting stream to the medium-term retention tier creates an audit integrity problem that no amount of GC correctness can fix. The tier assignment is a domain decision, and it must be made with explicit understanding of the consequences.
