...
**Quality target:** Universal Planning Framework B-grade at draft, A-grade after council review
...
Operates directly on `chapters/<part>/<ch>.md` files on `main`. Four workstreams:
...
### Phase 1 — Voice-agent tuning (all six agents)

Edit `~/.claude/agents/voice-*.md` files. For voice-sinek specifically:
...
For each guest agent (gladwell, brown, grant, godin, lencioni): apply the same chapter-scale audit. The fixes are likely lighter than Sinek's. Per-agent decisions, not one-size-fits-all.

#### Gate

- All six agent files updated and committed
- Previous versions preserved in git history
- `python build/voice-pass.py --plan-only` parses cleanly
...
- Add a third column to `chapters/voice-plan.yaml`: per-chapter pass-2 mode (`polish` or `normalize`). Current parser splits on `:` only — needs extension to parse `chapter: voice mode` or `chapter: voice` (mode defaults to `normalize` if absent for backward compat).
...
- Decide whether Appendix F gets voice-passed at all — recommendation: skip (it's a coverage table, not prose). Add `appendix-f` to `voice-plan.yaml` with a `skip` voice or omit entirely.
...
- All PROMOTE chapters merged on `main`
...
| 1 | Frontmatter syntax check on each agent file; `python build/voice-pass.py --plan-only` parses | User reads diff for each tuned agent; confirms new rules don't contradict existing ones |
| 2 | Per-pilot orchestrator OK; both pass-1 and pass-2 files exist; word count ±10% | User reads pass-1 and pass-2 for each pilot; PASS/FAIL with notes; audiobook listen on ≥1 pilot |
| 3 | Per-chapter OK; word count check on every draft; all 27 chapters have fresh `_voice-drafts/final/` file | Spot check N≥6 chapters across pipeline types |
| 4 | After promotion: `make word-count`, `make draft-pdf`, `make epub`, audiobook build | Per-chapter PROMOTE/REJECT decision; ICM markers updated; uncertain chapters routed back through literary-board |
...
- **Phase 4:** first 3 promoted chapters breaking build halts promotion. Fix regression source before continuing.

### Replanning triggers

The plan itself needs to change (not just continue) if:
...
- 27 chapters × up to 2 passes = up to 54 Claude headless invocations.
- 12 guest-voice pass-1 + 27 pass-2 = 39 expected.
- Each pass reads + writes 5,000–9,000 tokens of chapter; ~$0.30–0.50 per pass on Sonnet.
- **Estimated total: $15–25** for full Phase 3 run. Confirmed after pilot.

### Stop-loss

If cost runs >2× initial estimate by end of Phase 3 (i.e., >$50), pause and reassess.
...
|---|---|---|
| Voice agents | `~/.claude/agents/voice-{sinek,gladwell,brown,grant,godin,lencioni}.md` | The agents being tuned |
...
## 13. Stage 0 — Alternatives Considered
...
| 5 CORE: Success Criteria | ✅ §2 with binary FAILED conditions |
| 5 CORE: Assumptions & Validation | ✅ §3 with Confidence Levels |
| 5 CORE: Phases | ✅ §4 with binary gates |
| 5 CORE: Verification | ✅ §5 with audiobook listener test |
| Stage 0: Discovery | ✅ §13 |
| Rollback Strategy | ✅ §6 |
| Risk / Kill Criteria | ✅ §7 |
| Resume Protocol + Tool Fallback | ✅ §8 |
| Budget & Resources | ✅ §9 |
| Reference Library | ✅ §10 |
| Knowledge Capture | ✅ §11 |
| Post-Completion Plan | ✅ §12 |
| Stage 1.5 Adversarial Hardening | ⏳ pending — council-reviewer pass after spec written |
