# Draft — Chapter 21: Operating a Fleet of Local-First Nodes
## Extension #11 — NEW Chapter 21 in NEW Part V "Operational Concerns"

---

<!-- ============================================================ -->
<!-- PART 1: Ch21 full chapter                                     -->
<!-- File path on chapter creation:                                -->
<!--   chapters/part-5-operational-concerns/ch21-operating-a-fleet.md -->
<!-- Target: ~6,500 words                                          -->
<!-- ============================================================ -->
...
<!-- ============================================================ -->
<!-- PART 2: Front-matter / part-divider notes                    -->
<!-- ============================================================ -->
...
*~6,500 words. Ongoing-operations chapter for fleet-scale deployments.
Part IV teaches you to ship; Part V teaches you to sustain.*

### Chapter 21 — Operating a Fleet of Local-First Nodes
*~6,500 words*
- Fleet management as a distinct discipline; the headless-node operational model
- Provisioning at flash time: pre-provisioned hardware, zero-touch deployment,
  operator-mediated enrollment; enrollment receipts and fleet segmentation
- Fleet-scale key rotation orchestration: rotation epoch, propagation window,
  three-level straggler handling, decommission escalation
- OTA update coordination: signed bundles, staged rollouts, four verification
  gates, automatic rollback with pre-signed artifacts
- Fleet observability: signed heartbeat protocol, four dashboard dimensions,
  aggregation alerts, signed compliance snapshot export, five operational states
- The fleet failure scenario: 200 nodes, a key rotation, 17 stragglers, what
  fleet management makes visible that would otherwise be silent
```

Update the Word Count Targets table to include "Part V — Operational Concerns | ~6,500" and adjust the total accordingly. The current total (~84,700) becomes ~91,200.

Update the File Structure block to include the new directory and chapter file.

**3. `ASSEMBLY.md` update.** The chapter is `icm/draft` at this point and not yet eligible for inclusion in the assembly manifest. When the chapter advances to `icm/approved`, add it to ASSEMBLY.md in the position between Chapter 20 and the Epilogue.

**4. `docs/reference-implementation/sunfish-package-roadmap.md` update.** Advance the existing anticipatory entry `#11 fleet-management | NEW Sunfish.Foundation.Fleet` to a first-class entry per outline §C and §J. Status: `book-committed`. Architectural commitment: owns provisioning ceremony, fleet registry client, rotation epoch announcer and propagation tracker, OTA staging configuration and rollout tracker, heartbeat emitter (node-side), fleet aggregation and alerting (operator-side), compliance snapshot export. Open questions: whether `Sunfish.Foundation.Fleet` depends on `Sunfish.Kernel.Audit` or extends it; whether the fleet registry is operator-side only or has a node-side agent for high-trust scenarios.

This is the structural-change set required to land Chapter 21 cleanly. None of these updates are inside the chapter file itself; they are repository-level changes that the chapter-creation step must perform alongside writing the chapter file.

---

<!-- ============================================================ -->
<!-- PART 3: Reference-list entries (already inline in Ch21)      -->
<!-- ============================================================ -->

## Part 3: Reference-list entries

The eleven IEEE-numeric citations [1] through [11] appear in the chapter's References section above, in order of first appearance in the prose. They constitute Chapter 21's complete reference list as a new chapter.

Mapping to outline §G:

| Outline §G | Chapter ref | Topic |
|---|---|---|
| §G [1] | [1] | Apple Business Manager — provisioning §21.2 |
| §G [2] | [2] | Google Zero-Touch Enrollment — provisioning §21.2 |
| §G [3] | [3] | Windows Autopilot — provisioning §21.2 |
| §G [4] | [4] | TPM 2.0 — provisioning §21.2 |
| §G [5] | [5] | RAUC — OTA §21.4 |
| §G [6] | [6] | NIST SP 800-82 — context (referenced in §21.1 framing of regulated-tier fleets and ICS context, retained per outline §G even where not directly cited inline) |
| §G [7] | [7] | AICPA Trust Services Criteria — compliance export §21.5 |
| §G "verify" candidate | [9] | Mender — added per outline §D and §G "verify" list |
| §G "verify" candidate | [8] | Prometheus exposition format — added per outline §D and §G "verify" list |
| §G "verify" candidate | [11] | HIPAA Security Rule — added per outline §D and §G "verify" list |
| New | [10] | AWS IoT Fleet Indexing — added per outline §D candidate; supports §21.5 aggregation pattern |

The technical-review pass per outline §D should verify each citation's URL and may suggest additional sources or replace pre-existing ones. Reference [6] (NIST SP 800-82) is included in the reference list per outline §G even though its inline use in the current draft is implicit in the regulated-tier-fleet framing rather than a direct quotation; the technical reviewer may either pin it to a specific in-prose citation or mark it for removal.
...
**Word count (chapter body, prose only — excludes HTML comments, code fences, tables, headings markup, reference list, and Part 2/Part 3 meta-sections):**

Approximate body word counts by section (counted from the chapter's prose, not from this meta-document):

- Opening framing (before §21.1): ~700 words
- §21.1 Why fleet management is a distinct discipline (incl. FAILED block): ~720 words
- §21.2 Sub-pattern 11a Provisioning at flash time: ~1,180 words
- §21.3 Sub-pattern 11b Fleet-scale key rotation orchestration: ~1,180 words
- §21.4 Sub-pattern 11c OTA update coordination: ~1,160 words
- §21.5 Sub-pattern 11d Fleet observability: ~1,070 words
- §21.6 Fleet failure scenario: ~700 words

Chapter total: ~6,710 words. Within ±10% of 6,500 target.

**Sub-pattern 11a–11d coverage:**

- 11a (provisioning at flash time): §21.2 covers all three provisioning models, enrollment receipt, fleet segmentation. Cross-references to Ch15, Ch17, Ch19 wired.
- 11b (fleet-scale key rotation orchestration): §21.3 covers rotation epoch, propagation window with named default (72h), three escalation levels (warning / read-only quarantine / decommission), regulated-tier cadence, idempotent re-wrapping requirement. Cross-references to Ch15, ext #45, ext #48 wired.
- 11c (OTA update coordination): §21.4 covers update bundle, staged rollout (canary 1–5%, waves 25/50/100, soak windows), four verification gates, pre-signed rollback, air-gap delivery, per-deployment-class configurability. Cross-references to Ch19 wired.
- 11d (fleet observability): §21.5 covers heartbeat protocol (5-min default), four dashboard dimensions, aggregation alerts, compliance snapshot with SOC 2 Type II caveat, five operational states (Online current / Online stale / Offline / Quarantined / Decommissioned). Cross-references to Ch11, Ch14, ext #43 wired.

**FAILED conditions block (§21.1):** Rendered as a bold-labeled bulleted list under bold **FAILED conditions:** with named consequences and kill trigger sentence following. Not buried in prose.

**Cross-references wired (per outline §H):**

- Ch21 → Ch15 §Key Rotation and Revocation — wired in §21.1 framing and §21.3 opening
- Ch21 → Ch15 §Offline Node Revocation and Reconnection — wired in §21.3 read-only quarantine
- Ch21 → Ch17 §QR-Code Onboarding — wired in §21.2 operator-mediated enrollment
- Ch21 → Ch19 §Code Signing and Notarization — wired in §21.4 update bundle signing
- Ch21 → Ch19 §Admin Tooling — wired in §21.1 framing and §21.2 fleet registry
- Ch21 → Ch19 §Air-Gap Deployment — wired in §21.4 air-gap delivery
- Ch21 → Ch11 §The UI Kernel (`SunfishNodeHealthBar`) — wired in §21.5 dashboard
- Ch21 → Ch14 §Sync Daemon Protocol — wired in §21.5 heartbeat protocol
- Ch21 → Ch4 §Choosing Your Architecture — wired in §21.6 closing
- Ch21 → ext #43 (performance contracts) — wired in §21.5 four dimensions and §21.4 canary monitoring
- Ch21 → ext #45 (collaborator revocation) — wired in §21.5 security-posture dimension
- Ch21 → ext #48 (recovery-event audit trail) — wired in §21.3 audit substrate
- Ch21 → ext #9 (chain-of-custody) — wired in §21.2 enrollment receipt
- Ch21 → ext #47 (endpoint-compromise) — wired in §21.3 out-of-band rotation trigger

**Sunfish reference policy:**

- All references to `Sunfish.Foundation.Fleet` are by package name and appear inside `// illustrative — not runnable` code fences. The package name is forward-looking; no class APIs, method signatures, or constructor parameters are committed.
- Existing canon namespaces (`Sunfish.Kernel.Security`, `Sunfish.Kernel.Sync`) referenced by name only.
- Forward-looking namespaces from earlier extensions (`Sunfish.Kernel.Audit`, `Sunfish.Foundation.Recovery`) referenced by name only with the same illustrative posture.
- The roadmap entry advancement is documented in Part 2 of this draft.

**CLAIM markers inserted:** None. All architectural claims trace either to cited sources [1]–[11] or to the design-decisions §5 #11 architectural commitment. The Sub-pattern 11b idempotent re-wrapping requirement is flagged in outline §D for the technical-review pass to confirm against the kernel's existing key-rotation implementation; this is an implementation-conformance concern rather than a prose claim.

**QC checklist:**

- [x] QC-1 Word count within ±10% of 6,500 target (~6,710 words).
- [x] QC-2 All outline §A.1 through §A.6 sections addressed; FAILED conditions block included; sub-pattern coverage map verified.
- [x] QC-3 Source citations inline at first use, IEEE numeric, [1]–[11] in first-appearance order.
- [x] QC-4 Sunfish packages by name only; class names and method signatures appear only inside `// illustrative` code fences.
- [x] QC-5 No academic scaffolding ("this chapter presents", "in what follows", "as we will see") — verified.
- [x] QC-6 No re-introduction of the architecture; chapter assumes Parts I–IV. Opening explicitly chains to Ch19.
- [x] QC-7 Hybrid spec/tutorial register per outline §I — Ch19 voice precedent. Direct second-person address for operational instructions; specification voice for primitive definitions.
- [x] QC-9 N/A (not a council chapter).
- [x] QC-10 No placeholder text in the chapter body. Part 2 meta-section contains structural notes for the chapter-creation step, not placeholder prose.

**Items deferred to human voice-check (outline §F):**

- Opening hook anecdote: outline §F lists three candidates (the site-office manager whose application has not updated in six weeks; the security audit flagging 37 devices on outdated software; the rotation that completes on 194 of 200 nodes with no one knowing which six). Current draft opens with the connective-tissue sentence ("Chapter 19 got you to production. This chapter keeps you there.") and the construction-deployment frame; voice-check pass can fold in a felt anecdote at the top.
- The provisioning-as-cultural-shift moment in §21.2 — the transition from "managing users" to "operating infrastructure". Currently surfaced in the opening framing paragraph 7; voice-check can sharpen.
- Sinek register calibration per `feedback_voice_sinek_calibration.md` memory — light pacing only; do not over-mechanise.
- Optional connective-tissue line at the close of §21.6 explicitly naming Part V's role as the operational coda. Currently surfaced in the closing two paragraphs; voice-check can sharpen if needed.

**Forward-looking namespace dependencies (for code-check stage):**

- `Sunfish.Foundation.Fleet` is NEW. The code-check report must advance the existing anticipatory entry in `docs/reference-implementation/sunfish-package-roadmap.md` to first-class status with the architectural commitment specified in outline §C and Part 2 of this draft.
- `Sunfish.Kernel.Audit` (forward-looking, ext #48) and `Sunfish.Foundation.Recovery` (forward-looking, ext #48) are referenced by name. The code-check report should confirm the namespace decisions in extension #48 remain stable; Ch21 extends the same audit substrate and recovery patterns rather than introducing new ones.
- `Sunfish.Kernel.Performance` (forward-looking, ext #43) is referenced through the performance-posture dimension in §21.5. The code-check report should confirm the cross-reference is consistent with the ext #43 ch11/ch20 sections.
