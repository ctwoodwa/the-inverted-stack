...
Revocation without key rotation is weaker than the architecture can deliver. If the revoked collaborator's copy of the current KEK (Key Encryption Key) remains valid, they can still decrypt data re-encrypted after their departure. Key rotation closes this window.
...
4. The background DEK (Data Encryption Key) re-wrapping job processes documents in scope, wrapping each DEK under the new KEK.
5. The old KEK is discarded after all authorized members confirm receipt of the new bundle. The revoked node cannot decrypt documents re-wrapped under the new KEK.
