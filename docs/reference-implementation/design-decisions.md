...
3. **`failed-conditions: [<list>]`** per concept — explicit list of conditions that mean this concept is NOT met. Surfaced by universal-planning review as Stage 1 CORE requirement. Without this, conformance scoring can't distinguish "passes" from "doesn't actually pass but exists as code." Example for `ch11:NODE-13 Local event log`: `failed-conditions: [event log can be silently truncated; events written without monotonic ordering; no replay-safety test]`.
4. **`kill-triggers: [<list>]`** per primitive (Volume-level, not per-concept) — escalation criteria when conformance regresses. Example for P1 no-spinners: `kill-triggers: [conformance regresses below 95% for 3 consecutive sprints; any user-facing operation crosses 100ms ceiling]`. Surfaced by universal-planning Stage 0 Check 0.11 (kill criteria for zombie projects).
...
### 4.6.5 Out of current book scope — long-now governance (cognition + framing, not just substrate)

```yaml
- id: wales-future-generations-act-style-governance
  apps: [anchor × N (public-body systems), bridge × 1 (commissioner-oversight), thin-client-write × civil-society + future-stakeholder-advocates]
  notes: |
    Public body governance modeled on Wales's Well-being of Future
    Generations Act 2015. Multi-horizon impact analysis #40a; future-
    stakeholder representation #40b (Future Generations Commissioner
    statutory standing); reversibility-class tagging #40c with pacing
    delays #40d for irreversible; constitutional-style amendment
    thresholds #40e; slow-feedback dashboards #40f on soil / aquifer /
    infrastructure / institutional trust / biodiversity.
  catalog-coverage-estimate: ~15-25%
  new-primitives-needed: [#40 long-horizon decision governance, #42 optionality preservation]

- id: climate-policy-long-horizon-commitments
  apps: [anchor × N (jurisdictions), bridge × 1 (treaty secretariat), thin-client-read × civil-society + science-bodies]
  notes: |
    Multi-decade climate commitments. Constitutional-style constraints
    #40e (harder to amend than ordinary policy). Slow-feedback
    instrumentation #40f on CO2 budget / ocean acidification / ice-
    sheet / ecosystem indicators. Future-stakeholder representation
    #40b via intergenerational-justice advocates. Optionality
    preservation #42 — refuse irreversible mitigation pathways
    without strong justification. Inter-generational narrative
    preservation #41b — preserve scientific reasoning, not just
    targets.
  catalog-coverage-estimate: ~10-20%

- id: pension-fund-multi-generational-discipline
  apps: [anchor × 1 (pension/sovereign-wealth fund), thin-client-write × actuaries + trustees + future-pensioner-advocates, legacy-bridge × regulators + audit]
  notes: |
    40-80 year horizon fund. Actuarial discipline #40a in investment
    policy. Constitutional-style constraints #40e on withdrawal rules.
    Slow-feedback #40f on inflation / demographics / return assumptions.
    Future-pensioner representation #40b for currently-young + unborn
    workers. Decision-context preservation #41c — future trustees
    reconstruct WHY each decision. Optionality preservation #42 —
    refuse over-commitment to single asset class / strategy.
  catalog-coverage-estimate: ~25-35%
```
...
Volume 5 has TWO distinct sub-clusters that work together but address orthogonal failure modes. The Long Now Foundation framing makes clear that the long-now problem is fundamentally a COGNITION / GOVERNANCE / FRAMING problem (how do people and institutions think and act on multi-century horizons), with technical archival as one PART of it. A system that preserves bits perfectly but encourages quarterly thinking doesn't solve the long-now problem; it just makes the artifacts of short-term thinking more durable.

#### 5A — Technical substrate (data + custody + semantics survive)
...
#### 5B — Governance and cognition (decisions consider the long horizon)

The Long Now Foundation's core insight: the long-now problem is fundamentally a COGNITION / GOVERNANCE / FRAMING problem. Sub-problems: temporal myopia (markets/media/politics optimize for next quarter / next election while climate/ecosystems/infrastructure move on multi-decade-to-century scales); attention and culture (info environment rewards immediacy and outrage, not patience and stewardship); institutional half-life (most organizations / laws / business models not designed to remain legitimate over centuries); optionality vs. collapse (degenerative volatility closing off future options rather than opening new ones).

40. **Long-horizon decision governance** (Volume 5B long-now-governance, with civic-governance + climate-policy + infrastructure-lifecycle + intergenerational-justice + sovereign-debt as cross-cutting domains) — architectural mechanisms for encoding long-horizon thinking into systems that operate on short-horizon cycles. Addresses TEMPORAL MYOPIA + INSTITUTIONAL HALF-LIFE. Sub-patterns: 40a multi-horizon impact analysis (every significant decision logged with explicit reasoning across 1y / 10y / 100y / 1000y impact; cannot be marked complete without populating each horizon), 40b future-stakeholder representation (formal architectural representation of stakeholders who don't yet exist; modeled on Wales's Well-being of Future Generations Act 2015 statutory Future Generations Commissioner or indigenous seventh-generation principles), 40c reversibility budget (decisions tagged with reversibility class; bias toward reversibility for irreversible long-horizon impact), 40d pacing protocols (system-level mechanisms intentionally slow decision cycles to match consequence timescale; central-bank-independence pattern), 40e constitutional-style constraints (long-horizon commitments harder to change than the things they constrain; tiered governance with amendment thresholds proportional to commitment horizon), 40f slow-feedback instrumentation (system-level metrics on slow-moving variables — soil quality, aquifer depletion, infrastructure aging, institutional trust, climate indices — that current decision systems ignore because feedback is slower than decision cycles). Sources: Wales Well-being of Future Generations Act 2015; Future Generations Commissioner (Wales / Hungary / Israel); indigenous seventh-generation principles (Iroquois Confederacy); central-bank independence; constitutional supermajorities (US 2/3 + 3/4 states; Germany Article 79); pension-fund actuarial discipline (40-80 year horizons); Norwegian Sovereign Wealth Fund; climate-treaty ratchet mechanisms (Paris Agreement Art 4); CO2 budget accounting; aquifer-depletion monitoring (Ogallala, Central Valley); infrastructure-deficit accounting.

41. **Mythic-depth artifacts and narrative continuity** (Volume 5B long-now-governance, with cultural-heritage + civic-governance + education + indigenous-knowledge-preservation as cross-cutting domains) — cultural / cognitive mechanisms that make long-term consequences EMOTIONALLY VISIBLE in short-term cognition. Addresses ATTENTION AND CULTURE failure mode. Different from #39 deep-time semantics (which preserves the data) — #41 preserves and propagates the MEANING-MAKING practices that let humans care about distant horizons. Sub-patterns: 41a charismatic long-term artifacts (physical or institutional objects designed to make long-term horizons tangible and provocative; Clock of the Long Now is canonical example), 41b inter-generational narrative preservation (stories that carry meaning across centuries — origin stories, founder narratives, cautionary tales, mythic models; without these, systems lose the WHY of their commitments within a generation or two), 41c decision-context preservation (preserve the EVIDENCE, ARGUMENTS, AND REASONING that informed today's decisions — not just outcomes; future generations can reconstruct WHY we decided X, not just WHAT we decided), 41d future-naming patterns (make future stakeholders concrete by naming them — "What would my great-granddaughter Sarah, born 2087, think of this decision?"), 41e cultural heartbeat rituals (regular rituals that make long-term horizons present in short-term life — anniversaries, decadal reviews, century-marker commemorations), 41f living traditions of stewardship (communities of practice that transmit long-term thinking across generations; apprenticeship models within institutional structures). Sources: The Long Now Foundation (Clock of the Long Now, Rosetta Project); Brian Eno on the long now; Stewart Brand's Pace Layers; indigenous oral history (Aboriginal songlines, 60,000+ years of transmitted geographic knowledge); religious institution longevity (continuous traditions 1000-2500+ years); craft guild traditions (Japanese sword-smithing, Italian violin-making — multi-century apprentice lineages); national mythology; academic discipline traditions; family-business succession narratives.

42. **Optionality preservation across generations** (Volume 5B long-now-governance, with civic-governance + ecological-policy + economic-policy as cross-cutting domains) — architectural commitment to preserving FUTURE OPTIONS rather than optimizing present outcomes. Addresses OPTIONALITY VS. COLLAPSE failure mode (degenerative volatility closing off future options rather than opening new ones). Sub-patterns: 42a reversibility-of-decisions audit (track which decisions can be undone, which cannot, system-wide accounting), 42b format-stability commitments (resistance to gratuitous breaking changes; backward compatibility windows measured in decades not quarters), 42c vendor-substitution paths (always preserve the ability to switch implementations — engine-agnostic interfaces, open formats, exportable data), 42d low-volatility commitment registry (explicit registry of commitments that resist short-term optimization pressure; cannot be reversed without invoking the reversibility-budget process), 42e resilience over efficiency (architecturally bias toward resilient configurations even when less efficient — multiple independent paths, redundant custody, diverse algorithms; resists pressure toward monoculture optimization), 42f anti-monopoly enforcement (architecturally resist concentration that would close off alternatives — no single relay accumulates >X% of network traffic; no single CRDT engine accumulates >X% of deployments; structural pressure toward diversity). Sources: Nassim Taleb on antifragility and "via negativa"; biodiversity preservation principles; financial-market circuit-breakers; EU competition law (Article 102 TFEU dominant-position regulation); open-standards advocacy (W3C, IETF process); right-to-repair legislation; Sherman / Clayton Act antitrust traditions; "too big to fail" critiques; backward-compatibility commitments in long-lived protocols (TCP/IP, X.509).

### Volume 1 extensions surfaced by universal-planning review

The universal-planning rigor (Stage 0 contextual checks + Stage 1 CORE structure + Stage 1.5 adversarial hardening + Stage 2 anti-pattern scan) applied to each Kleppmann property surfaced 6 net-new primitives that the prior gap audit missed. All 6 fit Volume 1 — they're things the CURRENT BOOK should address, not future volumes.

43. **Performance contracts with framework-level enforcement** (Volume 1 extension; serves P1 no-spinners) — universal "no operation blocks UI thread" rule + per-operation latency budgets (writes <16ms, reads <Yms, sync <Zms) + measurable conformance tests. Stronger than mere "performance budgets" because it includes main-thread isolation guarantee for slow CRDT merge under load (real-world Yjs merges on 100k-op documents take seconds; UI must not freeze). Sub-patterns: 43a per-operation latency budget per operation class, 43b main-thread isolation guarantee (heavy work goes to web worker / background thread), 43c progressive-degradation fallback (when operation genuinely needs time, partial results visible immediately with completion indicator), 43d measurable conformance test in CI (per-operation budget violation fails the build), 43e per-deployment-class budget calibration (gaming has different budgets than document editing has different from email). FAILED conditions: any operation crosses budget; any UI freeze >16ms; any path requires network for core function. Kill trigger: conformance regresses below 95% for 3 consecutive sprints. Sources: Apple HIG 16ms frame budget; Web Vitals (LCP/FID/CLS/INP); Linear's 60fps obsession; React/Vue performance budget patterns; Yjs documented merge complexity; Replicache documentation on local-first responsiveness; Browser performance API + observability tooling.

44. **Per-data-class device-distribution policy** (Volume 1 extension; serves P2 multi-device) — explicit declarations of which devices in a personal fleet hold which data classes. Intentional locality as first-class concept, not something to fight against. Refines the personal-device-fleet pattern beyond "everything everywhere" — some data SHOULD be device-local (RAW photo files only on desktop, not phone; sensitive work docs only on managed corp device, not personal phone; ephemeral session data on watch only). Sub-patterns: 44a per-data-class device-eligibility declaration (which device classes can hold this data), 44b per-data-class storage-budget policy (RAW photos limited to 100GB on phone; rest stays on desktop), 44c selective sync UI (user controls per-class distribution), 44d device-loss recovery distinguishing "intentionally device-local lost" from "accidentally lost from sync mesh", 44e per-device capability descriptor (what this device can store / process / display). FAILED conditions: any data class accessible only on one device by accident (vs. by intentional policy); any device addition that requires data re-entry; any handoff that loses in-progress work. Sources: Apple iCloud Drive selective sync; Dropbox selective sync; Google Drive offline files; Lightroom catalog management; corporate-managed-device policies (BYOD vs. COPE); per-device storage budgets in mobile OSes.

45. **Collaborator revocation and post-departure data partition** (Volume 1 extension; serves P4 collaboration + P7 ownership) — handle the case where a previously-shared collaboration relationship breaks. #18 delegated capability handles GRANT; #45 handles UNGRANT + post-departure data partition + cached-copy management. Critical because P4 currently assumes collaborators stay collaborative. Sub-patterns: 45a explicit revocation event (cryptographically attested grant termination), 45b post-revocation key rotation (re-encrypt shared data under new keys; revoked collaborator's old keys no longer decrypt new state), 45c cached-copy management (revoked collaborator's local cache still readable for already-synced data — architecture is honest about this, doesn't claim impossible "remove from their device"; legal layer enforces destruction), 45d revocation propagation (other peers learn of revocation and stop accepting writes from revoked collaborator), 45e data-partition for divorce/dispute (when shared data must be split between parties — not just one revoked, but a controlled fork with each party getting their slice), 45f revocation-event audit trail (legally-defensible record of when revocation occurred, who authorized it, what data was affected). FAILED conditions: revoked collaborator can still write to shared state; revocation doesn't propagate to other peers within reasonable time; no audit trail of revocation event. Sources: departing-employee revocation (insider threat literature); divorce / partnership dissolution; board-removal of corporate access; OAuth token revocation; OCSP / CRL certificate revocation; AWS IAM credential rotation; Google Workspace account suspension procedures.

46. **Forward secrecy + post-compromise security** (Volume 1 extension; serves P6 security) — Signal-protocol-family standard cryptographic properties currently ASSUMED in the catalog but not explicitly named as primitive. Forward secrecy: past communications stay safe even if current keys compromised. Post-compromise security: future communications stay safe after key rotation following compromise. Standard in modern crypto (Signal protocol, MLS, Noise framework variants); not specifying creates risk that implementations skip them. Sub-patterns: 46a per-message ephemeral key derivation (each message uses fresh key derived from ratchet; old keys discarded), 46b sender + receiver ratchet (Double Ratchet pattern from Signal), 46c automatic key rotation triggered by suspected compromise OR by scheduled cadence, 46d sealed sender (recipient learns the message but relay learns nothing about sender), 46e protocol-level forward secrecy commitment (declared in protocol spec; testable). FAILED conditions: past messages decryptable from current keys; current key compromise compromises future messages without rotation; no automatic key rotation. Sources: Signal protocol (Marlinspike + Perrin); Double Ratchet algorithm; MLS (RFC 9420); Noise framework; OTR (Off-The-Record Messaging); Lavabit's Dark Mail (DIME); WhatsApp end-to-end encryption (Signal-based); ProtonMail symmetric ratchet.

47. **Endpoint-compromise threat model** (Volume 1 extension; serves P6 security; bridges to Volume 2 cyber-physical for hardened cases) — distinct from network/relay/supply-chain threats. What does P6 actually protect when the user's device is fully owned by an attacker? Honest answer matters; over-claiming protection is dangerous. Sub-patterns: 47a explicit endpoint-compromise scope declaration (catalog of what is + isn't protected when endpoint compromised), 47b hardware-security-module (HSM) / Secure Enclave separation (protects keys even with OS-level compromise; iPhone Secure Enclave model), 47c attested boot + integrity measurement (TPM-based proof that endpoint is running expected software), 47d remote-wipe capability (when endpoint loss detected, remotely revoke + crypto-shred — limited usefulness if attacker is offline), 47e endpoint-compromise containment (limit blast radius — compromised endpoint can't access OTHER users' data; can't impersonate other devices; can't backdate transactions), 47f honest documentation of post-compromise risk (architecture explicitly states what protection LAPSES at endpoint compromise — vs. silent failure). FAILED conditions: endpoint compromise can be silently used to impersonate other devices; compromised endpoint can backdate transactions; endpoint compromise scope undocumented. Sources: Apple Secure Enclave architecture; Pixel Titan M security chip; Windows Pluton; Trusted Platform Module (TPM); Intel SGX (and its subsequent vulnerabilities — cautionary); ARM TrustZone; mobile MDM remote-wipe; corporate disk-encryption (BitLocker, FileVault); hardware security keys (YubiKey, Titan, Solo); journalist-targeted malware analysis (Pegasus, Predator, Hermit reports).

48. **Key-loss recovery** (Volume 1 extension; serves P7 ownership) — most common P7 failure mode in real-world local-first deployments. User loses their key → loses all data. Currently scattered across catalog (#32c social recovery, Ch15 references); needs unification as primitive. Without this, P7 promise breaks at first password forgotten / device lost / death without succession. Sub-patterns: 48a multi-sig social recovery for individuals (smaller-scale variant of #32c — 3 of 5 friends help recover, time-locked), 48b custodian-held backup key (institutional backup with strong attestation requirements for release; user's lawyer / bank / cloud-custodian-of-choice), 48c paper-key fallback (printed recovery phrase stored offline; classic crypto-wallet pattern), 48d biometric-derived secondary key (face / fingerprint can derive recovery key under specific conditions), 48e timed-recovery with grace period (claim recovery; original holder has 7-30 days to dispute; only then transfer occurs), 48f recovery-event audit trail (cryptographically signed record of recovery event, who attested, what was recovered). FAILED conditions: user loses keys = loses all data with no recovery path; recovery so easy that attacker can also recover; no audit trail of recovery events. Sources: Argent wallet social recovery; Safe / Gnosis Safe multi-sig wallets; Vitalik Buterin "Why we need wide adoption of social recovery wallets" (2021); Apple iCloud Keychain recovery; Google Account recovery flow; password manager master-password recovery patterns; cryptocurrency seed phrase practices; bank safety deposit box backup model.

> **Universal-planning review note:** all 6 of these primitives (#43-#48) were surfaced by applying Stage 0 contextual checks + Stage 1.5 adversarial hardening (Pessimistic Risk Assessor, Skeptical Implementer, Devil's Advocate, Pedantic Lawyer, The Manager, Outside Observer) to each Kleppmann property. They were NOT surfaced by the prior less-rigorous gap audit. Two systemic findings from the same review: (1) catalog has rich "what" but thin "how-to-verify" — addressed by `failed-conditions` schema field added in §1.3 #3; (2) catalog had assumed several patterns that adversarial review challenged (P2 everything-everywhere, P4 collaborators-stay-collaborative, P7 keys-never-lost, P6 endpoint-equals-network) — addressed by the new primitives above.
...
```yaml
# New fields per concept entry (Path B additions):
...
failed-conditions:                                  # explicit list of conditions that mean concept is NOT met
  - <condition that means concept fails>
  - <another failing condition>
kill-triggers:                                      # primitive-level (not per-concept) escalation criteria
  - <condition that triggers architectural escalation>
  - <e.g., "conformance falls below 95% for 3 consecutive sprints">
```

The first two surfaced by app-archetype design discussion; the latter two surfaced by universal-planning review (Stage 1 CORE requirement for measurable success criteria + kill criteria for zombie-project anti-pattern).
...
- Family genealogy + civic deeds archive + indigenous cultural heritage → surfaced 10,000-year timespan as three distinct sub-problems requiring three distinct primitives in Volume 5A technical substrate: technology drift (#37 long-horizon format + media stability), institutional drift (#38 multi-generational custody and governance), and knowledge drift (#39 deep-time semantics) — book's epilogue had flagged "long-now archival format" as open question; now formalized
- Reframe of "long now" from technical-archival to cognition-and-governance problem (Long Now Foundation framing) → surfaced THREE more primitives in Volume 5B governance: long-horizon decision governance (#40 — multi-horizon impact analysis, future-stakeholder representation, reversibility budget, pacing protocols, constitutional-style constraints, slow-feedback instrumentation), mythic-depth artifacts and narrative continuity (#41 — charismatic long-term artifacts, inter-generational narrative preservation, decision-context preservation, future-naming patterns, cultural heartbeat rituals, living traditions of stewardship), optionality preservation across generations (#42 — reversibility audit, format stability, vendor substitution, low-volatility commitment registry, resilience over efficiency, anti-monopoly enforcement). Critical insight: a system that preserves bits perfectly but encourages quarterly thinking doesn't solve the long-now problem; both technical substrate AND governance/cognition primitives are needed.
- Universal-planning framework (Stage 0 contextual checks + Stage 1 CORE structure + Stage 1.5 adversarial hardening + Stage 2 anti-pattern scan) applied to each Kleppmann property (P1-P7) → surfaced 6 net-new Volume 1 extension primitives (#43-#48) that the prior less-rigorous gap audit had missed: performance contracts with framework-level enforcement (#43 P1), per-data-class device-distribution policy (#44 P2), collaborator revocation and post-departure data partition (#45 P4+P7), forward secrecy + post-compromise security (#46 P6), endpoint-compromise threat model (#47 P6), key-loss recovery (#48 P7). Two schema improvements added to §1.3: `failed-conditions` per concept (Stage 1 measurable success criteria) + `kill-triggers` per primitive (Stage 0 zombie-project anti-pattern defense). Two systemic findings: catalog had assumed P2 everything-everywhere, P4 collaborators-stay-collaborative, P7 keys-never-lost, P6 endpoint-equals-network — all challenged by adversarial review.

The architecture has converged: 48 primitives across 5 volumes (with Volume 5 split into 5A technical-substrate and 5B governance-and-cognition; #43-#48 are Volume 1 extensions surfaced by universal-planning rigor). Cross-cutting application domains expanded to 14: civic-governance, medical-systems, estate-planning, financial-systems, journalist-source-protection, dissident-protection, research-ethics, civic-archives, cultural-heritage, scientific-data, family-genealogy, indigenous-knowledge-preservation, climate-policy, sovereign-finance. New scenarios either fit existing archetypes with calibration OR add to the cross-volume primitive list when genuinely novel. Time to execute.
